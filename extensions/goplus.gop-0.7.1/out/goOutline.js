...
exports.runGoOutline = exports.documentSymbols = exports.GoOutlineImportsOptions = void 0;
...
function runGoOutline(options, token) {
    return new Promise((resolve, reject) => {
        ...
        p = cp.execFile(gooutline, gooutlineFlags, { env: goEnv_1.toolExecutionEnvironment() }, (err, stdout, stderr) => {
            try {
                if (err && err.code === 'ENOENT') {
                    gopInstallTools_1.promptForMissingTool('go-outline');
                }
                if (stderr && stderr.startsWith('flag provided but not defined: ')) {
                    gopInstallTools_1.promptForUpdatingTool('go-outline');
