...
function activate(context) {
    ...
    let indent = vscode.commands.registerCommand('Alignment Preserving Indent', () => {
    ...
    let outdent = vscode.commands.registerCommand('Alignment Preserving Outdent', () => {
        let editor = vscode.window.activeTextEditor;
        if (!editor) {
            return;
        }
    ...
    let newline = vscode.commands.registerCommand('Alignment Preserving Newline', () => {
        let editor = vscode.window.activeTextEditor;
        if (!editor) {
            return;
        }
        editor.edit((edit) => {
            ...
            editor.selections.forEach(it => {
                ...
                if (it.start.character < indent) {
                    edit.delete(new vscode.Range(it.start.line, 0, it.start.line, it.start.character));
                    edit.insert(it.start, prefix);
                }
                // Delete selection and any whitespace after selection end.
                edit.delete(new vscode.Range(it.start.line, it.start.character, it.end.line, endent));
